...
$(document).ready(function() {
   ...
   IMP.init('imp96096391');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   ...
   IMP.request_pay({
       ...
       name : 'Habibi 가구 상품결제',	 // 코인충전 또는 order 테이블에 들어갈 주문명 혹은 주문 번호. (선택항목)원활한 결제정보 확인을 위해 입력 권장(PG사 마다 차이가 있지만) 16자 이내로 작성하기를 권장
       ...
       buyer_addr : '',  
       buyer_postcode : '',
  //    m_redirect_url : '192.168.50.65:9090/MyMVC/member/coinUpdateLoginUser.up?idx=${idx}&coin=${coinmoney}'  // 휴대폰 사용시 결제 완료 후 action : 컨트롤러로 보내서 자체 db에 입력시킬것!
   }, function(rsp) {
       /*
		   if ( rsp.success ) {
			   var msg = '결제가 완료되었습니다.';
	   ...
	   */
		...
		*/
		...
		//	opener.location.href = "javascript:goCoinUpdate('${idx}','${coinmoney}');";
			window.opener.goCartUpdate('${userid}','${sumtotalPrice}');
		//  $(opener.location).attr("href", "javascript:goCoinUpdate('${idx}','${coinmoney}');");
			
		    self.close();
			
        } else {
            location.href="/habibi.hb";
            alert("결제에 실패하였습니다.");
       }
